...
class BaseRepository(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    ----------
    db : AsyncSession
        Active SQLAlchemy asynchronous session for database interaction.
        Managed externally (e.g., via an async context manager) and injected at construction time.
    ...
    def __init__(self, db: AsyncSession, model: Type[ModelType]) -> None:
        ...
        ----------
        db : AsyncSession
            Active SQLAlchemy asynchronous session providing transactional context and connection management.
        model : Type[ModelType]
    ...
    async def create(self, obj_data: CreateSchemaType) -> ModelType:
        ...
        ------
        IntegrityError
            If the operation violates a database constraint (e.g., unique name conflict).
            The transaction is rolled back automatically in this case.
        """
    ...
    async def update(self, obj_id: int, obj_data: UpdateSchemaType) -> ModelType | None:
        ...
        in accordance with the application's data model philosophy 
        (e.g., `device` is updatable; `name` or composite keys are not).
        ...
        ------
        IntegrityError
            If the update violates a database constraint (e.g., unique index).
